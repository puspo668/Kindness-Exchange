...
$sql = "SELECT * FROM cloth_donation";
...
if (!empty($selectedDistrict)) {
    $stmt = $conn->prepare("SELECT * FROM cloth_donation WHERE district = ?");
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>".$row['phone']."</td>
                <td>".$row['email']."</td>
                <td>".$row['cloth_types']."</td>
                 <td>".$row['cloth_gender']."</td>
                <td>".$row['quantity']."</td>
              </tr>";
    }
    echo "</table>";
